...
# noinspection PyMissingOrEmptyDocstring,PyDefaultArgument,PyUnusedLocal
class TQDMProgressBar(Callback):
    ...
    def _initialize_progbar(self, hook, epoch, logs=None) -> None:
        ...
        if hook == 'train_overall':
            ...
            current_epoch_description = f'Epoch {epoch + 1}/{self.num_epochs}'
            if self.show_epoch_progress:
    ...
    def _clean_up_progbar(self, hook, logs) -> None:
        if hook == 'train_overall':
            if self.show_overall_progress:
    ...
    def _update_progbar(self, logs) -> None:
    ...
    def format_metrics(self, logs={}, factor=1) -> str:
