"""
MLSTRUCTFP BENCHMARKS - ML - MODEL - UTILS - PLOT - ARCHITECTURES - PIX2PIX PATCH GAN FLOOR PHOTO MODEL

Model plot.
"""

__all__ = ['Pix2PixPatchGANFloorPhotoModelPlot']
...
if TYPE_CHECKING:
    from MLStructFP_benchmarks.ml.model.architectures import Pix2PixPatchGANFloorPhotoModel


class Pix2PixPatchGANFloorPhotoModelPlot(GenericModelPlot):
    """
    PIX2PIX Model plot.
    """
    _model: 'Pix2PixPatchGANFloorPhotoModel'

    def __init__(self, model: 'Pix2PixPatchGANFloorPhotoModel') -> None:
        """
        Constructor.
    ...
    def samples(self, epoch: int, batch_i: int, save: bool = False, savename: str = '', **kwargs) -> None:
        ...
        if save:
            fig.savefig('%s/%d_%d.png' % (sample_dir, epoch, batch_i), bbox_inches='tight', pad_inches=0.05)
            plt.close()
        else:
            plt.show()
